...
#include "ui_zhemporacontroldialog.h"
...
std::list<std::string> ZHemporaControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZHemporaControlDialog::listMints;

ZHemporaControlDialog::ZHemporaControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZHemporaControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZHemporaControlDialog::~ZHemporaControlDialog()
{
    delete ui;
}

void ZHemporaControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHemporaControlDialog::updateList()
{
...
void ZHemporaControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHemporaControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZHempora_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHemporaControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZHemporaControlDialog::GetSelectedMints()
{
...
void ZHemporaControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
