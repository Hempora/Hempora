...
#include "zhemporacontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zHEMP ought to be enough for anybody." - Bill Gates, 2017
    ui->zHEMPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzHEMPSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzHEMP->setEnabled(false);
        ui->pushButtonMintzHEMP->setToolTip(tr("zHEMP is currently disabled due to maintenance."));

        ui->pushButtonSpendzHEMP->setEnabled(false);
        ui->pushButtonSpendzHEMP->setToolTip(tr("zHEMP is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zHEMPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzHEMP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHEMP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zHEMP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zHEMP in ") +
...
void PrivacyDialog::on_pushButtonSpendzHEMP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zHEMP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zHEMP
        sendzHEMP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zHEMP
    sendzHEMP();
}

void PrivacyDialog::on_pushButtonZHemporaControl_clicked()
{
    ZHemporaControlDialog* zHEMPControl = new ZHemporaControlDialog(this);
    zHEMPControl->setModel(walletModel);
    zHEMPControl->exec();
}

void PrivacyDialog::setZHemporaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzHEMPSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzHEMP()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Hempora Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zHEMPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zHEMP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " HEMPORA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zHEMP</b>";
    ...
    // use mints from zHEMP selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZHemporaControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZHemporaControlDialog::GetSelectedMints();
    }
    ...
    // Clear zhempora selector in case it was used
    ZHemporaControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zHEMP Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Hempora, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zHEMP Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zHEMP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zHEMP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zHEMP "));
    ui->labelzHEMPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzHEMPSyncStatus->setVisible(fShow);
}
